...
from data.surface_data import ForecastEnsembleSurfaceData, ForecastSurfaceData
        ...
        for i, file in enumerate(file_tuple):
            ...
            if raw_s2s_lats is None:
                ...
                if aux_raw_s2s_data is None:
                    ...
                    aux_raw_s2s_data = np.concatenate(
                        [aux_raw_s2s_data, precip], axis=1
                    )
...
def aggregate_ens_s2s_precip(lead_time_files, storm_dates):
    ...
    for lead_time, file_tuple in lead_time_files.items():
        ...
        for i, file in enumerate(file_tuple):
            ...
            precip = np.expand_dims(
                np.diff(ds.tp.values[idxs_to_keep_diff], axis=0), axis=0
            )
            precip = np.reshape(
                precip, (1, precip.shape[2], precip.shape[1], *precip.shape[3:])
            )
            precip = precip / 6
            precip = np.where(precip < 0, 0, precip)
            ...
            if raw_s2s_lats is None:
                ...
                if aux_raw_s2s_data is None:
                    ...
                    aux_raw_s2s_data = np.concatenate(
                        [aux_raw_s2s_data, precip], axis=2
                    )
