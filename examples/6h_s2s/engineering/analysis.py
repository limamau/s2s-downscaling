...
def plot_pre_processing(nearest, lowpass, cpc):
    ...
    figs_dir = os.path.join(script_dir, "figs")
    os.makedirs(figs_dir, exist_ok=True)
    fig.savefig(os.path.join(figs_dir, "pre-processing.png"))
...
def make_plots(
    ...
    nearest_det_s2s_file = os.path.join(test_data_dir, "det_s2s_nearest.h5")
    nearest_det_s2s = ForecastSurfaceData.load_from_h5(nearest_det_s2s_file, ["precip"])
    ...
    # nearest_ens_s2s = ForecastEnsembleSurfaceData.load_from_h5(
    #     nearest_ens_s2s_file, ["precip"]
    # )
    lowpass_ens_s2s_file = os.path.join(test_data_dir, "ens_s2s_nearest_low-pass.h5")
    lowpass_ens_s2s = ForecastEnsembleSurfaceData.load_from_h5(
        lowpass_ens_s2s_file, ["precip"]
    )
