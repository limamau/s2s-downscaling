...
class SurfaceData:
    ...
    def cut_data(self, extent):
        ...
        for var_name in self._var_names:
            setattr(
                self, var_name, getattr(self, var_name)[..., lat_idx, :][..., lon_idx]
            )
    ...
    def take_out_from_date_range(
        self, date_ranges: Tuple[Tuple[np.datetime64, np.datetime64], ...]
    ):
        ...
        new_sfc_data = self.__class__(
            ...
            self.longitude,
            **{
                var_name: getattr(self, var_name)[..., ~date_idx, :, :]
                for var_name in self._var_names
            },
        )
